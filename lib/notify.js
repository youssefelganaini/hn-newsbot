...
const {AI_THRESHOLD} = process.env;
const interestingThreshold = AI_THRESHOLD;
    ...
    for (const article of importantArticles) {
        ...
        try {
            if (index && reason) {
                ...
                const markdown = `Source: ${serviceMd}**${title}** - [${hostname}](${link})${reason && index ? `\n> AI rating ${index}/100: ${reason}` : ""
                }`;


                //Send alert via email
                await email("New Article found from Corbado bot", markdown)
                ...
                console.log(`Article ${article_id} (${title}) does not have a valid index and reason. Email not sent.`);
            }
        } catch (error) {
            console.log(error)
        }
