...
const {OPENAI_API_KEY, OPENAI_ORGANIZATION_ID, AI_THRESHOLD} = process.env;
const configuration = new Configuration({apiKey: OPENAI_API_KEY, organization: OPENAI_ORGANIZATION_ID});
const openai = new OpenAIApi(configuration);

const MAX_RETRIES = 3; // Maximum number of retries
const RETRY_DELAY = 1000; // Delay between retries (2 seconds)
const RATE_LIMIT_DELAY = 1000; // Delay between requests (2 seconds)
...
const askChatGPT = async ({prompt, retries = MAX_RETRIES}) => {
    try {
         await sleep(RATE_LIMIT_DELAY); // Ensure a delay between requests
        ...
        if (error.response?.status === 429 && retries > 0) { // Check for 429 status code
            console.log("Rate limit exceeded. Retrying...");
             await sleep(RETRY_DELAY); // Wait before retrying
            return askChatGPT({prompt}, retries - 1); // Retry the request
        }
        console.log(error);
        return "";
    }
...
module.exports.interestingIndexArticle = async ({content}) => {
    ...
    try {
        ...
        keywords_secondary = "[vue, angular, react, nextjs, javascript, nuxtjs, svelte, php, node, golang]";
        ai_threshold = AI_THRESHOLD;
        const prompt = `We are a business called Corbado (corbado.com). It's a startup which adds future-proof, 
        passwordless authentication in <1h to new and existing web and native applications. Let your users login with passkeys 
        via Face ID, Touch ID, Android Biometrics or Windows Hello to boost conversion and signup rates, improve UX 
        and increase security. 
        We like articles about: people writing about WebAuthn / passkeys / (passwordless) authentication, competitors 
        in the (passwordless) authentication space, something related to data security benefits of WebAuthn / passkeys, 
        passkey / WebAuthn technical implementation guides and tutorials or when traditional password-based 
        authentication gets negative press. 
        Some of our competitors are Clerk (clerk.dev), Hanko (hanko.io), OwnID (ownid.com), Passage (passage.id)
        or Auth0 (auth0.com). Is the following article interesting for us? Reply with a single number between 0 and 100.
        0 being not interesting at all, 100 being super interesting. The article MUST include these keywords ${keywords_primary} to have a 
        rating >= ${ai_threshold}, even better in combination with ${keywords_secondary}. If ${keywords_primary} are not included, 
        the rating MUST be lower than ${ai_threshold}, no exceptions.
