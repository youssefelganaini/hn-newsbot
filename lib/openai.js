...
module.exports.interestingIndexArticle = async ({link, content}) => {
    try {
        ...
        const prompt = `We are a business called Corbado (corbado.com). It's a startup which adds future-proof, passwordless authentication in <1h to new applications and existing systems. Let your users login with passkeys via Face ID or Touch ID to boost conversion / signup rates, improve UX and increase security. We like articles about: people writing about WebAuthn / passkeys / (passwordless) authentication, competitors in the (passwordless) authentication space, something related to data security benefits of WebAuthn / passkeys, passkey / WebAuthn technical implementation guides or when traditional password-based authentication gets negative press. Some of our competitors are Clerk (clerk.dev), Hanko (hanko.io), OwnID (ownid.com) or Auth0 (auth0.com). Is the following article interesting for us? Reply with a single number between 0 and 100. 0 being not interesting at all, 100 being super interesting. After this number, add a dash (-), following by a short explanation. This is the crawled article:\\n\\n$\{content}`;
